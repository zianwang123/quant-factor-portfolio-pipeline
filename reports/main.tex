...
\begin{abstract}
We build an end-to-end quantitative factor investment pipeline that constructs 20 cross-sectional equity factors from Compustat/CRSP data (1970--2019), selects a parsimonious subset via four complementary methods, and allocates across factor portfolios using mean-variance optimization, risk parity, and Black-Litterman posterior returns. Out-of-sample (2015--2019), our best variant (IC-Weighted) achieves a Sharpe ratio of 1.31 on a market-neutral basis, compared to 0.91 for the S\&P~500 and 0.56 for a hedge fund index composite. Rolling backtests with transaction costs confirm robustness: equal-weight and risk parity strategies maintain Sharpe ratios above 1.1 net of 10~bps costs. Black-Litterman improves both MVO and Max Sharpe by regularizing extreme weights, while adaptive annual factor re-selection underperforms fixed selection---demonstrating the cost of factor chasing.
\end{abstract}
...
Table~\ref{tab:oos} reports OOS performance (January 2015 -- December 2019) for all portfolio variants and benchmarks.
...
\midrule
\multicolumn{8}{l}{\textit{Our Portfolios}} \\
IC-Weighted     & \textbf{1.311} & 2.740 & 4.3\%  & 3.3\%  & $-$1.9\%  & $-$1.2\% & $-$1.2\% \\
MVO + BL        & 1.242          & 2.632 & 5.7\%  & 4.6\%  & $-$4.5\%  & $-$1.5\% & $-$1.6\% \\
Equal Weight    & 1.201          & 2.109 & 3.7\%  & 3.1\%  & $-$3.2\%  & $-$1.2\% & $-$1.2\% \\
MVO             & 1.164          & 2.159 & 8.5\%  & 7.3\%  & $-$7.5\%  & $-$2.5\% & $-$2.6\% \\
Max Sharpe + BL & 1.157          & 2.142 & 7.5\%  & 6.5\%  & $-$7.4\%  & $-$2.4\% & $-$2.2\% \\
Max Sharpe      & 1.082          & 1.994 & 10.0\% & 9.3\%  & $-$10.4\% & $-$3.4\% & $-$3.3\% \\
Risk Parity     & 1.005          & 1.713 & 3.0\%  & 3.0\%  & $-$3.0\%  & $-$1.2\% & $-$1.2\% \\
\midrule
...
S\&P 500           & 0.907 & 1.219 & 10.8\% & 12.0\% & $-$14.1\% & $-$6.0\% & $-$5.2\% \\
FF Mkt-RF          & 0.857 & 1.103 & 10.7\% & 12.5\% & $-$15.1\% & $-$6.1\% & $-$5.5\% \\
Smart Beta (EW)    & 0.744 & 0.925 & 8.9\%  & 11.9\% & $-$16.4\% & $-$5.5\% & $-$5.3\% \\
...
\begin{itemize}[noitemsep]
    \item All seven of our variants achieve Sharpe $> 1.0$, outperforming the S\&P~500 (0.91) on a risk-adjusted basis.
    \item Our portfolios are market-neutral (long-short QSpreads), so absolute returns are lower than fully invested benchmarks, but drawdowns are substantially smaller.
    \item IC-Weighted (Sharpe 1.31) is the simplest and best-performing variant. This echoes the finding that simple, robust allocations often beat optimized ones out of sample.
    \item MVO + BL (Sharpe 1.24) outperforms plain MVO (1.16), showing that BL regularization helps by shrinking returns toward equilibrium.
...
We test whether our portfolios statistically outperform benchmarks using the Jobson-Korkie test with Memmel correction. For IC-Weighted vs S\&P~500, the difference in Sharpe ratios (1.31 vs 0.91) is economically meaningful, though statistical significance depends on the short OOS window (60 months).
...
\midrule
Equal Weight      & 1.188 & \textbf{1.184} & 3.6\% & $-$3.1\% & 4.9\%  & 55  \\
Risk Parity       & 1.149 & 1.143          & 3.3\% & $-$2.7\% & 5.5\%  & 60  \\
Max Sharpe + BL   & 0.939 & 0.913          & 4.2\% & $-$6.0\% & 38.1\% & 423 \\
MVO + BL          & 0.919 & 0.892          & 4.6\% & $-$7.1\% & 27.7\% & 307 \\
Max Sharpe        & 0.838 & 0.808          & 4.8\% & $-$6.5\% & 47.3\% & 525 \\
IC-Weighted       & 0.744 & 0.718          & 3.2\% & $-$9.4\% & 18.7\% & 207 \\
MVO               & 0.720 & 0.691          & 6.4\% & $-$13.6\%& 55.4\% & 615 \\
IC-Wt + BL        & 0.714 & 0.688          & 3.2\% & $-$8.4\% & 18.3\% & 203 \\
\bottomrule
\end{tabular}}
\end{table}
...
\textbf{Key findings:}
\begin{itemize}[noitemsep]
    \item Equal Weight and Risk Parity are the most robust---low turnover ($< 6\%$) translates to minimal transaction cost drag ($\sim$55--60 bps total over the backtest).
    \item \textbf{BL improves MVO}: Net Sharpe rises from 0.69 to 0.89, while turnover drops from 55\% to 28\%. BL shrinks extreme MVO weights toward equilibrium, acting as a natural regularizer.
    \item \textbf{BL improves Max Sharpe}: Net Sharpe rises from 0.81 to 0.91, turnover drops from 47\% to 38\%.
    \item MVO without BL has the highest turnover (55\%) and cost (615 bps), consistent with the known instability of unconstrained mean-variance portfolios.
\end{itemize}
...
\midrule
MVO         & 0.691 & 0.892 & +0.200 & 55.4\% $\to$ 27.7\% \\
Max Sharpe  & 0.808 & 0.913 & +0.105 & 47.3\% $\to$ 38.1\% \\
IC-Weighted & 0.718 & 0.688 & $-$0.030 & 18.7\% $\to$ 18.3\% \\
\bottomrule
\end{tabular}
\end{table}

BL benefits optimizers that are sensitive to return estimation (MVO, Max Sharpe) but offers no improvement for IC-Weighted, which already uses a simple, robust weighting scheme.
...
\midrule
Equal Weight  & 1.184 & 0.727 & 3 & 37 bps \\
IC-Weighted   & 0.718 & 0.619 & 3 & 60 bps \\
MVO + BL      & 0.892 & 0.580 & 3 & 103 bps \\
\bottomrule
\end{tabular}}
\end{table}
...
Our market-neutral portfolios exhibit substantially lower drawdowns than long-only benchmarks. IC-Weighted's maximum drawdown is 1.9\%, compared to 14.1\% for the S\&P~500 and 21.1\% for the equal-weight mutual fund basket. This is a direct consequence of market neutrality: our returns are driven by cross-sectional dispersion rather than market direction.
...
\begin{enumerate}
    \item \textbf{Five robust factors} (AccrualRatio, CFTP, STReversal, AssetGrowth, ROE) survive multi-method selection and deliver consistent OOS alpha.

    \item \textbf{Simple allocations are hard to beat}: IC-Weighted (Sharpe 1.31) and Equal Weight (1.20) outperform sophisticated optimization in the static OOS test; Equal Weight (1.18) dominates in the rolling backtest after costs.

    \item \textbf{Black-Litterman regularizes optimization}: BL improves MVO's net Sharpe by +0.20 and reduces turnover by half, demonstrating its value as a return shrinkage estimator rather than a standalone allocation method.

    \item \textbf{Factor stability matters}: Fixed factor selection outperforms annual adaptive re-selection, supporting the use of economically motivated, stable factor portfolios over data-mined alternatives.

    \item \textbf{Transaction costs are material}: MVO's gross Sharpe of 0.72 falls to 0.69 net of costs; more aggressive strategies like Max Sharpe lose 3--5\% of annualized return to turnover.
\end{enumerate}
